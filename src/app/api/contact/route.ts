...
const limiter = rateLimit({
  ...
  uniqueTokenPerInterval: 500 // Max unique IPs tracked
})
...
function generateCompanyEmail(data: ContactFormData): string {
  return `
    <!DOCTYPE html>
    <html lang="en">
      <head>
  ...
  return `
    ...
    <html lang="en">
      <head>
        ...
        <title>Thank You - NewGen Strata Services</title>
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.7; color: #333; margin: 0; padding: 0; background-color: #f4f4f4; }
          .container { max-width: 600px; margin: 20px auto; background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
          .header { background: linear-gradient(135deg, #006633, #6B1D7F); color: white; padding: 40px 20px; text-align: center; }
          .header h1 { margin: 0 0 10px 0; font-size: 28px; }
          .content { padding: 35px; }
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p>Thank you for contacting <strong>NewGen Strata Services</strong>. We have received your inquiry and appreciate you taking the time to reach out.</p>
            ...
            <p><strong>Need to speak with us urgently?</strong></p>
            <p>Phone: <a href="tel:+61399999999" style="color: #006633; text-decoration: none; font-weight: 600;">+61 3 9XXX XXXX</a></p>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const companyEmailOptions = {
      ...
      html: generateCompanyEmail(validatedData),
      replyTo: validatedData.email,
    }
    ...
    const customerEmailOptions = {
      ...
      to: validatedData.email,
      subject: 'Thank you for contacting NewGen Strata Services',
      html: generateCustomerEmail(validatedData),
    }
